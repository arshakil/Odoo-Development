...
class RestApi(http.Controller):
    ...
    def _check_credentials(self, kwargs):
        ...
        if not set(self._VALIDATION_FIELDS)<set(kwargs.keys()):
            return False, False, self.get_response(401, '401', {'error': {'code': '401', 'message': 'Authentication Required 1'}})
        ...
        if not user:
            return False, False, self.get_response(401, '401', {'error': {'code': '401', 'message': 'Authentication Required 2'}})
        return key, user, False

    @http.route(['/restapi/1.0/common/oauth1/request_token/grant'], \
        type="http", auth="public", csrf=False, website=True)
    def auth_request_token_grant(self, **kwargs):
        return "Grant"

    @http.route(['/grant'], \
        type="http", auth="public", csrf=False, website=True)
    def request_grant(self, **kwargs):
        return "Grant"
