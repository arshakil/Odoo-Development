...
class EarnLeaveRequest(models.Model):
    ...
    allotted_days = fields.Integer(string="Allotted Days", readonly=1)
    remaining_days = fields.Integer(string="Remaining Days", readonly=1)
    requested_days = fields.Integer(string="Requested Days", readonly=1)
    ...
    def calculate_allotted_days(self):
        if self.earn_leave_user_id:
            ...
            check_annual_assign_days = self.env['annual.leave.assign'].search([])
            print ('Assign Days is : ',check_annual_assign_days.annual_leave_days)
            assign_days = check_annual_assign_days.annual_leave_days
        ...
        if self.earn_leave_user_id:
            ...
            if check_remaining_exists:
                ...
                for e in check_remaining_exists[0]:
                    check_allotted_days = self.env['earn.leave'].search(
                        [("earn_leave_user_id", "=", self.earn_leave_user_id.name), ])
                    for a in check_allotted_days[0]:
        ...
        if self.earn_leave_user_id:
            ...
            if check_remaining_exists:
                ...
                for e in check_remaining_exists[0]:
                    ...
                    annual_remaining_days = new_allotted_days
    ...
    def action_refuse(self):
        ...
        check_remaining_exists = self.env['annualleave.remaining'].search([("leave_user_name", "=", self.earn_leave_user_id.name), ])
        for e in check_remaining_exists[0]:
            print ('leave taken exist: ',e.leave_taken)
            # taken_leave = abs(e.leave_taken-e.leave_user_remaining_days)
            taken_leave = abs(e.leave_taken-self.requested_days)
            self.remaining_days = remaining_days
            print ('remaining days: ',self.remaining_days)
            print ('leave taken less:: ',taken_leave)
            annualRemainingLeave = self.env['annualleave.remaining'].browse(self.env.context.get('active_ids', []))
    ...
    def create(self, values):
        ...
        if annual.requested_days>annual.remaining_days or annual.remaining_days==0:
